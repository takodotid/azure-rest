import type { AzureCredential, AzureToken } from "./lib/AzureCredential.js";
...
export type AzureClientOptions = {
	...
	credential: {
		...
		scope: string;
		builder?: (token: AzureToken) => Record<string, string>;
	};
};
...
export class AzureClient {
	...
	public async sendRequest(path: string, init?: RequestInit): Promise<Response> {
		...
		return fetch(`${this.options.baseUrl}${path}`, {
			...init,
			headers: {
				...(this.options.credential.builder ? this.options.credential.builder(this.token) : { Authorization: `Bearer ${this.token.token}` }),
				...init?.headers
			}
		});
	}
